...
def setupdb(url):
	...
	c.execute('CREATE TABLE read_art (id integer primaty key, group_id integer, is_read integer)') # 0 -- read, 1 -- to read
	conn.commit()

	if DEBUG == 1:

		c.execute("INSERT INTO servers VALUES (NULL, 'news.gmane.org', NULL, NULL)")
		c.execute("INSERT INTO groups VALUES (NULL, 1, 'gmane.comp.python.committers', 0, 0, 0, 0)")
